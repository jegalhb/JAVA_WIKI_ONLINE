...
flowchart LR
    ...
    모드 -- 예 --> 온라인초기화[클라이언트 연결 및 서버 데이터 동기화]
    모드 -- 아니오 --> 오프라인초기화[로컬 JSON 데이터 로드]
    ...
    상세표시 --> 작업여부{추가/수정/삭제 작업이 있는가?}
    작업여부 -- 아니오 --> 종료([종료])
    작업여부 -- 예 --> 저장반영[저장소 반영\nConceptRepository]

    저장반영 --> 전파여부{온라인 모드인가?}
    전파여부 -- 예 --> 서버전파[서버 전파\nADD/UPDATE/DELETE]
    전파여부 -- 아니오 --> 로컬유지[로컬 상태 유지]
...
flowchart TD
    시작[검색 시작] --> 키입력[키워드 입력]
    키입력 --> 타이머[디바운스 타이머 재시작]
    타이머 --> 추천계산[추천 목록 계산]

    추천계산 --> 추천여부{추천 목록 비어있음?}
    추천여부 -- 예 --> 팝업숨김[추천 팝업 숨김]
    추천여부 -- 아니오 --> 팝업표시[추천 팝업 표시/갱신]

    팝업표시 --> 선택여부{추천 선택했는가?}
    선택여부 -- 예 --> 단건렌더[선택 항목 1건 렌더링]
    선택여부 -- 아니오 --> 일반검색실행[일반 검색 실행]

    팝업숨김 --> 일반검색실행
    단건렌더 --> 상세동기화[상세 패널 동기화]
    일반검색실행 --> 결과판단{검색 결과 존재?}

    결과판단 -- 예 --> 결과출력[검색결과 트리 출력]
    결과판단 -- 아니오 --> 유사추천[유사 항목 안내 출력]
...
sequenceDiagram
    ...
    사용자->>편집창: 개념 추가/수정/삭제 요청
    편집창->>메인화면: 결과 전달(onDataAdded 등)
...
- `검색결과` 노드를 먼저 구성해 카테고리 확장 없이 결과 확인이 가능합니다.
- 온라인/오프라인 분기 이후 처리(서버 전파 여부)를 다이어그램에서 명확히 구분했습니다.
## 6. 실행 방법
