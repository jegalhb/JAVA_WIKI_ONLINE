# JAVA_WIKI Improved Version

Java 학습 개념을 검색/조회/추가/수정/삭제하고, JSON 저장과 실시간 협업 채팅을 지원하는 Swing 프로젝트입니다.

이번 개선의 핵심은 단순 기능 추가가 아니라, **기존 방식의 문제를 발견하고 실제 사용 흐름 기준으로 해결한 과정**입니다.

## 1. 개선 배경 (기존 -> 개선)

### 기존 구조의 문제
- `data.txt` 중심 저장 방식이라 구조화/검증/확장이 어려움
- 카테고리 버튼이 다수로 분산되어 UI 동선이 복잡함
- 검색은 실행 후 결과 확인 방식이라 입력 중 탐색 경험이 느림
- 검색 추천을 선택해도 유사 결과가 너무 많이 나와서 정확 탐색이 어려움
- 한글 조합 입력(IME) 중 자동완성 이벤트와 충돌해 입력이 끊기는 현상 발생
- 검색 점수/임계값이 매직 넘버로 분산되어 정책 변경 시 수정 지점이 많음
- query/title/tag 전처리 규칙이 분산되어 비교 결과 일관성이 흔들릴 수 있음
- 검색 정렬용 중간 객체(`SearchResult`)가 필드 직접 접근 구조라 캡슐화가 약함

### 개선 방향
- 저장 포맷을 `data.json`으로 전환
- 카테고리 선택을 콤보박스로 단일화
- 검색 자동완성(입력 중 추천) 도입
- 기본 Enter 검색은 `searchByKeyword`(입력어 중심)로 고정하고, 추천은 방향키 선택 후 Enter로 확정
- IME 입력을 방해하지 않도록 키/포커스 처리 정리
- 검색 점수/임계값 상수화로 정책 변경 지점을 단일화
- `normalizeQuery`/`normalizeText` 공통화로 전처리 규칙을 통일
- `SearchResult` 불변 객체화 + getter 접근으로 안정성과 캡슐화 강화

## 1-1. 변경 전/후 한눈에 비교

| 항목 | 기존 방식 | 개선 방식 | 개선 효과 |
|---|---|---|---|
| 데이터 저장 | `data.txt` 텍스트 중심 | `data.json` 구조화 저장 | 파싱/검증/확장 용이 |
| 카테고리 UI | 버튼 다중 배치 | `JComboBox` 단일 선택 | 화면 단순화, 동선 축소 |
| 검색 경험 | 실행 후 결과 확인 | 입력 중 자동완성 + 추천 팝업 | 탐색 속도 향상 |
| 추천 선택 동작 | 선택 후 유사 결과 다수 노출 | 선택 키워드로 즉시 재검색 + 검색창 동기화 | 추천어 탐색과 일반 검색 의도 분리 |
| 한글 입력 안정성 | IME 조합 중 간헐적 끊김 | debounce + 이벤트 분리 | 연속 입력 안정화 |
| 검색 결과 접근 | 카테고리 수동 확장 필요 | `Search Results` 노드 즉시 표시 | 결과 접근 시간 단축 |
| 상세 연동 | 검색 후 추가 클릭 필요 | 추천 선택 즉시 상세 동기화 | 확인 단계 감소 |
| 점수 정책 유지보수 | 매직 넘버 분산(10.0/8.0/5.0/3.0 등) | 상수화(`private static final`) | 정책 변경/튜닝 시 수정 지점 최소화 |
| 문자열 전처리 일관성 | 메서드별 trim/lower/null 처리 편차 | `normalizeQuery`/`normalizeText` 공통화 | 비교 기준 일관성 향상, 입력 노이즈 감소 |
| 검색 중간 객체 설계 | `SearchResult` 필드 직접 접근 | `private final` + getter 불변 객체 | 캡슐화 강화, 스트림 파이프라인 안전성 향상 |

## 2. 이번에 추가/수정된 검색 기능

### 2-1. 자동완성 추천 UI
- `DocumentListener` + `Timer`(debounce)로 입력 중 추천 계산
- 추천 목록은 `JPopupMenu + JList`로 표시
- `UP/DOWN/ESC` 키로 추천 목록 탐색 가능

### 2-2. 추천 선택 시 추천어 확정 검색
- 변경 전: 추천 선택 직후 동작이 일반 검색과 섞여 Enter 의도가 모호함
- 변경 후: 추천 선택 시 `searchField`를 추천어로 변경하고 `runKeywordSearch(selected.getTitle(), true)` 실행
- 결과: 추천어로 재검색된 목록을 보여주며 검색창 키워드도 동일하게 유지

### 2-3. 검색 결과 바로 확인
- 검색 모드에서 트리에 `Search Results` 노드를 추가
- 카테고리(기초/중급/고급/메서드)를 일일이 펼치지 않아도 검색 결과를 즉시 확인 가능


### 2-4. 검색 점수 정책 리팩토링 반영 요약
- SearchService 점수 로직의 매직 넘버를 상수화해 유지보수성을 높였습니다.
- query/title/tag 비교에 공통 정규화 규칙(trim + Locale.ROOT 소문자)을 적용했습니다.
- calculateScore의 null/blank 방어를 강화해 런타임 안정성을 높였습니다.
### 2-5. 네이버/구글형 Enter UX 정렬
- 기본 Enter/검색 버튼: 현재 입력어 기준 `searchByKeyword` 실행
- 추천 팝업에서 방향키 탐색 후 Enter: 추천어 확정 검색
- 추천 팝업이 떠 있어도 방향키 탐색을 하지 않았다면 Enter는 일반 검색 유지

## 3. 실제 이슈와 해결 기록

### 이슈 A. 한글 입력이 중간에 멈춤 (`크`/`클` 이후 입력 불가)
원인
- 자동완성 동작 중 검색 실행/포커스 이벤트가 IME 조합 흐름에 간섭

해결
- 자동완성은 문서 변경 시 debounce로만 갱신
- Enter 동작을 분리: 추천이 열려 있고 선택이 있으면 추천 수락, 아니면 일반 검색
- 프로그램적으로 텍스트를 변경할 때 `suppressAutoCompleteUpdate`로 재진입 방지

### 이슈 B. Enter 의도 충돌(일반 검색 vs 추천 선택)
원인
- 추천 팝업이 떠 있을 때 Enter 처리 의도가 섞여 일반 검색/추천 선택 구분이 불명확

해결
- `suggestionNavigatedByKeyboard` 플래그로 방향키 탐색 여부를 추적
- Enter 기본 동작은 `performSearch()`(키워드 검색)로 유지
- 방향키 탐색 후 Enter일 때만 `acceptSuggestionSelection()` 실행

### 이슈 C. 검색 후 결과 확인을 위해 폴더를 계속 열어야 함
원인
- 검색 모드 전용 결과 노드가 없어 카테고리 트리 의존

해결
- `renderTree(List<Concept> concepts, boolean searchMode)`로 확장
- `searchMode=true`일 때 `Search Results` 노드 생성 및 결과 배치


### 이슈 D. SearchService 점수 계산 리팩토링 (null 방어 + 정책 상수화)
원인
- 점수 계산 로직에 하드코딩 값(매직 넘버)이 분산되어 있어 유지보수 시 수정 지점이 많았다.
- query/title/tag 정규화 기준이 메서드별로 분산되어 비교 결과 일관성이 떨어질 수 있었다.
- concept/title/tags/tag null 입력에서 NPE가 발생할 수 있는 경로가 존재했다.

해결
- 점수/임계값을 `private static final` 상수로 통일해 정책 변경 지점을 단일화했다.
- `normalizeQuery`, `normalizeText`를 도입해 trim/lowercase/null-safe 규칙을 공통화했다.
- `calculateScore`에서 null/blank를 선제 차단하고 tags/tag null 방어를 적용했다.
- `getBestMatch`, `suggest`도 동일한 정규화/상수 정책을 사용하도록 맞췄다.
### 이슈 E. 추천어 선택 후 검색창 키워드 동기화 필요
원인
- 추천어를 선택해도 검색창/결과 키워드 동기화가 명확하지 않아 흐름이 끊길 수 있음

해결
- `acceptSuggestionSelection()`에서 `searchField.setText(selected.getTitle())`로 검색창을 먼저 동기화
- 이어서 `runKeywordSearch(selected.getTitle(), true)`를 호출해 추천어 기준 결과로 즉시 갱신
## 4. 코드 반영 위치
- 메인 검색/자동완성/UI 제어(Enter 분기, 추천어 확정검색 포함): `src/Reproject/MainWikiFrame.java`
- 검색 점수/추천 계산 + 키워드 검색 API(`searchByKeyword`): `src/Reproject/SearchService.java`
- 데이터 저장/로딩(JSON): `src/Reproject/ConceptRepository.java`

## 5. 핵심 메서드 시퀀스

### 5-1. 입력 중 자동완성
```mermaid
sequenceDiagram
    autonumber
    participant U as User
    participant SF as searchField
    participant T as debounce Timer
    participant MW as MainWikiFrame
    participant SS as SearchService

    U->>SF: 키 입력
    SF->>T: restart()
    T-->>MW: timeout
    MW->>SS: suggest(query, limit)
    SS-->>MW: 추천 목록 반환
    MW->>MW: popup list 갱신
```

### 5-2. 추천 선택(추천어 확정 검색)
```mermaid
sequenceDiagram
    autonumber
    participant U as User
    participant MW as MainWikiFrame

    U->>MW: 추천 클릭/방향키+Enter
    MW->>MW: acceptSuggestionSelection()
    MW->>MW: runKeywordSearch(selected.getTitle(), true)
```

### 5-3. 일반 검색 실행 + 트리 렌더링 (`performSearch` + `renderTree`)
```mermaid
sequenceDiagram
    autonumber
    participant U as User
    participant SF as searchField
    participant MW as MainWikiFrame
    participant SS as SearchService
    participant T as JTree

    U->>SF: 검색어 입력 후 Enter/검색 버튼
    SF-->>MW: performSearch()
    MW->>MW: suggestionPopup 숨김
    MW->>SS: searchByKeyword(keyword)
    SS-->>MW: List<Concept>
    MW->>MW: updateList(results, searchMode=true)
    MW->>MW: renderTree(concepts, searchMode)
    MW->>T: setModel(DefaultTreeModel)
    MW->>T: expandRow() 초기 확장
```

### 5-4. 서버 동기화 + 개념 추가 전파 (`applyServerData` + `onDataAdded`)
```mermaid
sequenceDiagram
    autonumber
    participant S as Server
    participant C as WikiClient
    participant MW as MainWikiFrame
    participant R as ConceptRepository
    participant E as ConceptEditFrame

    S-->>C: 최신 Concept 목록 전송
    C-->>MW: applyServerData(concepts)
    MW->>R: replaceAll(concepts)
    MW->>MW: 검색 상태에 따라 updateList 또는 refreshList

    E-->>MW: onDataAdded(concept)
    MW->>R: addConcept(concept)
    MW->>C: (온라인 모드) send("ADD", concept)
    MW->>MW: refreshList()
```

### 5-5. 현재 뷰 재계산 (`applyCurrentView`)
```mermaid
sequenceDiagram
    autonumber
    participant U as User
    participant MW as MainWikiFrame
    participant R as ConceptRepository
    participant SS as SearchService

    U->>MW: 카테고리 변경 / 화면 갱신 트리거
    MW->>MW: keyword 확인(searchField)
    alt keyword is empty
        MW->>R: findAll()
    else keyword exists
        MW->>SS: searchByKeyword(keyword)
    end
    MW->>MW: updateList(base, searchMode)
```

### 5-6. 자동완성 갱신 분기 (`updateAutoCompleteSuggestions`)
```mermaid
sequenceDiagram
    autonumber
    participant MW as MainWikiFrame
    participant SS as SearchService
    participant P as SuggestionPopup

    MW->>MW: suppressAutoCompleteUpdate 확인
    alt suppress=true
        MW-->>MW: return
    end
    MW->>MW: keyword trim
    alt keyword empty
        MW->>P: hide
        MW-->>MW: return
    end
    MW->>SS: suggest(keyword, limit)
    alt suggested empty
        MW->>P: hide
        MW-->>MW: return
    else suggested exists
        MW->>MW: model clear/add + selectedIndex=0
        MW->>P: show or revalidate/repaint
    end
```

### 5-7. 추천 확정 처리 (`acceptSuggestionSelection`)
```mermaid
sequenceDiagram
    autonumber
    participant U as User
    participant MW as MainWikiFrame

    U->>MW: 추천 클릭/방향키+Enter
    MW->>MW: selected 항목 확인
    alt selected is null
        MW-->>MW: return
    end
    MW->>MW: suppressAutoCompleteUpdate=true
    MW->>MW: searchField.setText(selected.getTitle())
    MW->>MW: suppressAutoCompleteUpdate=false
    MW->>MW: suggestionPopup hide
    MW->>MW: runKeywordSearch(selected.getTitle(), true)
```

### 5-8. 상세 패널 렌더링 (`displayDetail`)
```mermaid
sequenceDiagram
    autonumber
    participant T as JTree
    participant MW as MainWikiFrame
    participant SP as JScrollPane(detail)

    T-->>MW: TreeSelectionEvent(selected Concept)
    MW->>MW: detailPanel 생성
    MW->>MW: title/category/구분선 배치
    MW->>MW: descriptionLines 순회
    alt heading line
        MW->>MW: heading 스타일 적용
    else code line
        MW->>MW: code 스타일 적용
    else normal line
        MW->>MW: body 스타일 적용
    end
    MW->>SP: setViewportView(detailPanel)
    MW->>SP: scrollTop + revalidate/repaint
```

### 5-9. 로직 프로세스 (분기 상세)

#### 5-9-1. 시작부터 종료까지 전체 분기 상세도
```mermaid
flowchart TD
    A([시작]) --> B([연결 확인])
    B --> C{온라인 모드인가}

    C -- 예 --> D[서버 연결 및 동기화 데이터 수신]
    C -- 아니오 --> E[로컬 JSON 로드]
    D --> F[메인 화면 진입]
    E --> F

    F --> G[/검색어 입력/]
    G --> H[자동완성 갱신<br/>문서 변경 감지 + 디바운스]
    H --> I{추천 항목 존재 여부}

    I -- 예 --> J{추천 선택 여부}
    J -- 예 --> K[추천어로 검색 실행<br/>검색창 키워드 동기화]
    J -- 아니오 --> L[일반 검색 실행<br/>performSearch]
    I -- 아니오 --> L

    L --> M{검색 결과 존재 여부}
    M -- 예 --> N[검색결과 노드 렌더링<br/>renderTree searchMode true]
    M -- 아니오 --> O[유사 항목 안내<br/>getBestMatch]

    K --> P[상세 패널 표시<br/>displayDetail]
    N --> P
    O --> G

    P --> Q{추가 수정 삭제 작업 여부}
    Q -- 아니오 --> R([종료])
    Q -- 예 --> S[저장소 반영<br/>ConceptRepository]

    S --> T{온라인 모드 여부}
    T -- 예 --> U[서버 전파<br/>ADD UPDATE DELETE]
    T -- 아니오 --> V[로컬 상태 갱신]

    U --> W[목록 새로고침<br/>refreshList]
    V --> W
    W --> G

    classDef startEnd fill:#e9f7ef,stroke:#2e7d32,color:#1b5e20,stroke-width:1.5px;
    classDef process fill:#eef4ff,stroke:#2f5ea8,color:#183b73,stroke-width:1.2px;
    classDef decision fill:#fff4e5,stroke:#b26a00,color:#7a4300,stroke-width:1.2px;
    classDef warn fill:#fdecec,stroke:#b33939,color:#7a1f1f,stroke-width:1.2px;

    class A,R startEnd;
    class B,D,E,F,G,H,K,L,N,P,S,U,V,W process;
    class C,I,J,M,Q,T decision;
    class O warn;
```

#### 5-9-2. 데이터 저장/동기화 상세 시퀀스
```mermaid
sequenceDiagram
    autonumber
    participant 사용자
    participant 편집화면
    participant 메인화면
    participant 저장소
    participant 서버

    사용자->>편집화면: 개념 추가 수정 삭제 요청
    편집화면->>메인화면: 결과 전달 onDataAdded 등
    메인화면->>저장소: 데이터 반영 add update delete

    alt 온라인 모드
        메인화면->>서버: 변경 이벤트 전송
        서버-->>메인화면: 최신 목록 동기화
        메인화면->>저장소: replaceAll 반영
    else 오프라인 모드
        메인화면->>메인화면: 로컬 상태만 갱신
    end

    메인화면->>메인화면: refreshList 실행
    메인화면-->>사용자: 갱신된 목록 상세 화면 표시
```

프로세스 포인트
- 추천 선택 경로는 방향키+Enter일 때만 확정되어 Enter 의도가 명확합니다.
- 검색 결과는 `검색결과` 노드로 우선 노출되어 카테고리 확장 없이 확인할 수 있습니다.
- CRUD 이후 온라인 오프라인 분기에 따라 서버 전파 여부가 달라집니다.
## 6. 실행 방법
1. 서버 실행: `Reproject.WikiServer`
2. 클라이언트 실행: `Reproject.WikiClient`
3. 단독 실행(오프라인 테스트): `Reproject.Main`

## 7. 참고 화면
- 메인 UI: `docs/screenshots/main-ui.png`
- 수정/등록 UI: `docs/screenshots/edit-frame.png`
- 코드 라인 렌더링: `docs/screenshots/code-line-rendering.png`

## 8. SearchService 리팩토링 반영 (안정성/유지보수성)

### 8-1. 점수 정책 상수화
- 기존 하드코딩 점수(`10.0`, `8.0`, `5.0`, `3.0`)와 임계값(`0.5`)을 상수로 분리했습니다.
- 자동완성 가중치(`20.0`, `8.0`)와 Best Match 기준값(`0.5`)도 상수로 통일했습니다.
- 점수 정책 변경 시 `SearchService` 상단 상수만 수정하면 되도록 정리했습니다.

### 8-2. 검색어/텍스트 정규화 공통화
- `normalizeQuery(query)`: `trim + toLowerCase(Locale.ROOT)` 규칙을 공통 적용합니다.
- `normalizeText(value)`: null-safe 소문자 변환을 공통 적용합니다.
- `search/suggest/getBestMatch/calculateScore`가 동일한 비교 기준을 사용하도록 맞췄습니다.

### 8-3. null/blank 방어 강화
- `calculateScore()`에서 `concept/query` null 및 공백 query를 먼저 차단합니다.
- title/tag/tags null 가능성을 방어해 NPE 가능 경로를 줄였습니다.
- 유사도 계산도 공통 정규화를 사용해 로케일/대소문자 처리 일관성을 높였습니다.

### 8-4. 리팩토링 효과 요약
- 점수 계산 로직의 의미가 상수명으로 드러나 가독성이 좋아졌습니다.
- 입력 전처리 규칙이 한 곳으로 모여 동작 예측 가능성이 올라갔습니다.
- 이후 검색 튜닝 시 코드 수정 범위가 줄어 유지보수가 쉬워졌습니다.
### 8-5. 추가 리팩토링 반영 (정규화 보강 + SearchResult 불변화)
- `normalizeText(value)`에 `trim()`을 포함해 앞뒤 공백 노이즈를 비교 전에 제거하도록 보강했습니다.
- `SearchResult`를 `private final` 필드 + getter 구조의 불변 객체로 변경했습니다.
- 스트림 파이프라인(`search/suggest/getBestMatch`)도 필드 직접 접근 대신 getter 사용으로 캡슐화를 맞췄습니다.

### 8-6. Enter 검색 UX 개선 반영
- 기본 Enter/검색 버튼은 `searchByKeyword`로 동작해 입력한 단어 중심 결과를 보여줍니다.
- 추천어 확정은 방향키 탐색 후 Enter로만 처리되어 일반 검색과 의도가 분리됩니다.
- 추천어 확정 시 검색창 텍스트와 실제 검색 키워드가 항상 동일하게 유지됩니다.

